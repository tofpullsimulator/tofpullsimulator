...
@SpringBootTest(classes = {CreateMatrixCommand.class, BannerService.class})
@ComponentScan(basePackages = {"org.eos.tof.common"})
@EnableAutoConfiguration
class CreateMatrixCommandTest {

    @Autowired
    private CreateMatrixCommand command;
    @Autowired
    ...
    void shouldGetTheNameOfTheCommand() {
        Assertions.assertEquals("banner create-matrix", command.getName());
    }
    ...
    void shouldHandleInteractionEvent() {
        ...
        StepVerifier.create(mono)
                .assertNext(it -> Assertions.assertEquals(Banner.Spec.ZEKE, it.spec()))
                .verifyComplete();
    }
